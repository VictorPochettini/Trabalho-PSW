...
export default function ChallengesPage() {
  ...
  const matchesTab = (d) => {
    ...
    if (activeTab === "meus") return d.tipo === "comunidade" && String(d.criadorId) === String(currentUser?.id);
    return true;
  };
    ...
    <>
                ...
                <div key={d.id} className="ch-card">
                  ...
                  <div className="ch-info">
                    ...
                    <div className="ch-actions">
                      <button className="fy-btn-follow ch-cta" onClick={() => navigate(`/desafios/${d.id}`)}>Ver desafio</button>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <style>{`
        :root{
          ...
          --text-strong:#f7f8ff; --text-soft:rgba(255,255,255,.9); --muted:rgba(255,255,255,.72);
        ...
        .fy-title{
          margin:8px 0 6px; font-weight:800; letter-spacing:.2px; font-size:clamp(26px,2.4vw,34px);
        ...
        .fy-search-container{ max-width:700px; margin:6px auto 18px; }
        .fy-search-box{
          ...
          transition:all .25s ease; box-shadow: 0 8px 20px rgba(0,0,0,.15);
        }
        .fy-search-box:focus-within{ border-color:rgba(255,255,255,.38); box-shadow: 0 10px 26px rgba(0,0,0,.20); }
        .fy-search-icon{ color:var(--muted); margin-right:12px; font-size:1.1rem; }
        .fy-search-input{ flex:1; border:none; background:transparent; color:#fff; font-size:1rem; outline:none; }
        .fy-search-input::placeholder{ color:rgba(255,255,255,.7); }
        .fy-search-clear{ background:rgba(255,255,255,.10); border:1px solid rgba(255,255,255,.26); color:#fff; cursor:pointer; padding:6px; border-radius:10px; transition:all .2s ease; }
        .fy-search-clear:hover{ background:rgba(255,255,255,.18); }
        .fy-search-results-info{ display:flex; align-items:center; justify-content:center; gap:10px; margin-top:10px; color:var(--text-soft); }
        .fy-search-clear-btn{ background: rgba(255,255,255,.12); border:1px solid rgba(255,255,255,.24); color:#fff; padding:6px 12px; border-radius:12px; font-size:.9rem; cursor:pointer; transition:all .2s ease; }
        .fy-search-clear-btn:hover{ background:rgba(255,255,255,.18); border-color:rgba(255,255,255,.30); }

        .fy-filters{ display:grid; grid-template-columns:1fr auto; align-items:center; gap:14px; margin-bottom:16px; }
        ...
        .fy-filter-btn:hover{ transform:translateY(-1px); border-color:rgba(255,255,255,.30); background:linear-gradient(180deg, rgba(255,255,255,.24), rgba(255,255,255,.12)); box-shadow: 0 12px 28px rgba(0,0,0,.20); }
        .fy-filter-btn.active{ background:linear-gradient(135deg, var(--accent), var(--accent-2)); border-color:rgba(255,255,255,.28); box-shadow:0 12px 26px rgba(94,23,235,.32); }
        .fy-filter-btn:disabled{ opacity:.55; cursor:not-allowed; }
        ...
        .fy-sort select{
          ...
          border-radius:14px; padding:8px 12px; outline:none; box-shadow: 0 8px 18px rgba(0,0,0,.14);
        }
        ...
        .ch-create-btn{
          ...
          font-weight:800; letter-spacing:.2px; padding:10px 12px;
          background:linear-gradient(135deg, var(--accent), var(--accent-2));
          box-shadow:0 12px 26px rgba(94,23,235,.28);
          transition: transform .12s ease, filter .2s ease, box-shadow .25s ease, border-color .2s ease;
          cursor:pointer;
        }
        .ch-create-btn:hover{ transform:translateY(-1px); filter:brightness(1.05); border-color:rgba(255,255,255,.34); box-shadow:0 16px 32px rgba(94,23,235,.36); }
        .ch-create-btn:disabled{ opacity:.55; cursor:not-allowed; }

        .fy-loading{ color:var(--text-soft); }
        .fy-no-results{
          background: rgba(255,255,255,.12); border:1px solid rgba(255,255,255,.24);
          border-radius: 22px; padding:40px 20px; backdrop-filter: blur(14px); text-align:center; color:var(--text-soft);
        }
        .fy-no-results h4{ color:#fff; margin-bottom:10px; }
        ...
        .ch-card{
          ...
          overflow:hidden; transition: transform .18s ease, box-shadow .25s ease, border-color .22s ease;
        }
        .ch-card:hover{ transform:translateY(-2px); box-shadow: var(--shadow-2); border-color: var(--surface-hover-border); }
        .ch-thumb{ position:relative; aspect-ratio:16/9; overflow:hidden; }
        .ch-thumb img{ width:100%; height:100%; object-fit:cover; display:block; transition:transform .5s ease; }
        .ch-card:hover .ch-thumb img{ transform:scale(1.04); }

        .ch-info{ padding:14px 14px 16px; display:grid; gap:10px; }
        .ch-badges{ display:flex; gap:8px; flex-wrap:wrap; }
        .ch-badge{ font-size:.72rem; padding:.35rem .6rem; border-radius:999px; border:1px solid rgba(255,255,255,.28); background: rgba(255,255,255,.14); letter-spacing:.2px; text-transform:capitalize; }
        .ch-badge.oficial{ background:linear-gradient(135deg, rgba(99,102,241,.30), rgba(59,130,246,.26)); border-color:rgba(147,197,253,.45); }
        .ch-badge.comunidade{ background:linear-gradient(135deg, rgba(168,85,247,.28), rgba(236,72,153,.24)); border-color:rgba(232,121,249,.42); }
        .ch-badge.status.aberto{ background:rgba(34,197,94,.24); border-color:rgba(134,239,172,.45); }
        .ch-badge.status.encerrado{ background:rgba(148,163,184,.24); border-color:rgba(148,163,184,.45); }

        .ch-title{ font-weight:800; margin:2px 0 0; }
        .ch-desc{ margin:0; color:rgba(255,255,255,.92); min-height:2.6em; }
        .ch-meta{ display:flex; gap:14px; flex-wrap:wrap; color:rgba(255,255,255,.85); }
        .ch-meta-item{ display:flex; align-items:center; gap:8px; }
        .ch-actions{ display:flex; justify-content:flex-end; }
        .ch-cta{
          border-radius:14px; border:1px solid rgba(255,255,255,.28); color:#fff; font-weight:800; letter-spacing:.2px; padding:10px 12px;
          background:linear-gradient(135deg, var(--accent), var(--accent-2));
          box-shadow:0 12px 26px rgba(94,23,235,.28);
          transition: transform .12s ease, filter .2s ease, box-shadow .25s ease, border-color .2s ease; cursor:pointer;
        }
        .ch-cta:hover{ transform:translateY(-1px); filter:brightness(1.05); border-color:rgba(255,255,255,.34); box-shadow:0 16px 32px rgba(94,23,235,.36); }

        @media (max-width:640px){
          .fy-filters{ grid-template-columns:1fr; gap:12px; }
          .right-tools{ justify-content:space-between; }
          .fy-search-container{ margin-bottom:12px; }
          .ch-meta{ gap:8px; }
        }
      `}</style>
    </>
